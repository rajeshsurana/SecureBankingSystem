...
@Controller
public class PKIControllerForDelete {
	...
	@RequestMapping(value = "/generatedeletePKI.do", method = RequestMethod.POST)
	...
	@RequestMapping(value = "/verifyPKIForDelete.do", method = RequestMethod.POST)
			...
			if (privatekey != null) {
				...
				if (pkiverification != null) {
					if (pkiverification.getUserVerificationCode().equals(decryptData(messageDecoded, privatekey))) {
						...
						return new ModelAndView("PKIDecryptedTextForDelete", "pkiverification", pkiverification);
					} else {
						ModelAndView mvc = new ModelAndView("verifyPKIForDelete");
	...
	@RequestMapping(value = "/encryptPKIforDelete.do", method = RequestMethod.POST)
	public ModelAndView verifyPKI(@RequestParam(value = "pki") String pki, HttpServletRequest request, Model m,
		...
		if (pkiverification != null && (pkiverification.getUsername() != StaticConstants.ServerPrivateKeyUsername
			...
			try {
				...
				return new ModelAndView("PKIEncryptForDelete");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	@RequestMapping(value = "/verifyServerFordelete.do", method = RequestMethod.POST)
	public String ProccedToAction(@RequestParam(value = "encryptedText") String encryptedText,
			HttpServletRequest request, Model m, final RedirectAttributes redirectAttributes,
			...
			try {
				...
				if (tempPrivateKeyOfServer != null) {
					...
					if (pkiverification.getUserVerificationCode().equals(decryptData(messageDecoded, privatekey))) {
						...
						if (bankuser != null
								&& ((bankuser.getRefUserRole().getBankUserRoleName().equals(StaticConstants.Merchant))
							...
							if ((request.getSession().getAttribute("sUserName")) == null) {
								redirectAttributes.addFlashAttribute("messageLoggedOut",
								...
								if (!tempUser.getRefUserRole().getBankUserRoleName()
										.equals(StaticConstants.ExternalCustomer)
										&& !tempUser.getRefUserRole().getBankUserRoleName()
												.equals(StaticConstants.Merchant)) {
									redirectAttributes.addFlashAttribute("messageLoggedOut",
											"You have been redirected to login page due to authorization violation.");
									request.getSession().removeAttribute("sUserName");
									return "redirect:/";
								}
							}

							BankUser user = bankUserService
									.findUserByUserName((String) (request.getSession().getAttribute("sUserName")));

							bankUserService.updateAccountStatus(user.getUserName(), "RequireDeActivation");
							redirectAttributes.addFlashAttribute("deactiveUser",
									"User is in suspended state. Waiting approval from manager");
